...
For example,  we assumed that the  core motivation for this work
was widely known and accepted in the community. That was a mistake so this draft completely rewrites the Introduction
to include much more background information on the 
presence of the problem studied here.}

\response{Next, using reviewer feedback, we can now better frame
our  research questions (this has the added benefit
of clarifying our discussion).}
 
 
\response{Apart from these two points, we have also addressed
...
\item The novelty of the approach has  been clarified (R3). (see our comments to Reviewer3, in \bareresp{3-1},
below)
\item The study now includes all the details to ensure 
replicability   (R1, R2).
\item The results of the study were   be revisited and clarified. 
...
\response{You are quite correct, we did not define bellwether till very late in the prior draft. In this draft, we define bellwether on page 1\citeresp{1-1a}} where we say:

\begin{quote}
    \response{Given a community of projects developed by the same organization,
...
name of the  leading sheep of a flock, with a bell on its neck.}
\end{quote}
...
\response{Thank you for your comments.
 On review, we found that the core problem of conclusion instability
was not well documented in the introduction. This new draft fixes that problem with an extensively revised introduction~\citeresp{1-2} that better focuses the work on conclusion instability and scalability.}
...
text on page1 ~\citeresp{1-3a} takes care to document {\em why }
...
\begin{quote}
\response{
"The logistic regression learner,  since it is widely used in defect prediction~\cite{ghotra2015revisiting,zhang2017data,he2012investigation,nam2013transfer,pan2010domain} in   the  software engineering domain and, in our experience,
...
\begin{quote}
    \response{(we used)... The SMOTE class imbalance correction algorithm~\cite{Chawla2002} since many of the projects have relatively very few or a large number of defects. A model built on a project with such skewed class distribution can result in a biased model towards the majority class (Such models can either have a low recall or a high recall with high pf). Thus we run SMOTE on the training data to artificially boost the number of target examples in the training data and achieving balanced recall and pf for the minority class test examples}
\end{quote}
...
\response{You are quite correct, we did not include the details about how the features are being removed and why in the last draft. In this draft, we have included notes regarding how CFS works and why the features that are not used are removed \citeresp{1-5b}:}

\begin{quote}
    \response{Hall's CFS feature selector~\cite{hall1999correlation} is used to identify features which are highly correlated with the classification, yet uncorrelated to each other. The features which are  highly correlated with each other will not add more information while making classification, while the features which are uncorrelated towards the classification also not add more information while making classification. The CFS algorithm incrementally picks one feature at a time which add the most information towards the classification at a given step and stops when there is no more improvement in the score.}
\end{quote}

\response{As to what attributes were selected, please see Figure~\ref{fig:FSS_level1}. As to what of
the selected figures were most important, please see Table~\ref{tbl:coefs_4}.} 
...
\response{Thank you for that comment.  You are quite correct-- we did not document all the necessary citations for supporting our claim before. We have added the following note to \citeresp{1-5e}}

\begin{quote}
    {\color{blue}Also,   extensive studies have found that CFS more useful than many other feature subset selection methods such as  PCA or InfoGain or RELIEF\respto{1-5e} ~\cite{hall1999correlation,challagulla2008empirical,arar2015software,lee2016developer,rodriguez2007attribute,gao2015combining,hosni2017investigating}. }
\end{quote}
...
\response{Thank you for your comment, we didn't explain why these metrics need to be maximized, For those notes, please see \citeresp{1-5f} and \citeresp{1-5g}.}
...
\response{You are quite correct. In the last draft we indeed did not include a detailed analysis of the results to show the scalability of our model. In this draft \citeresp{1-2}\citeresp{1-6a} we have included 2 more research questions (RQ1 and RQ2) to show the conventional bellwether is slower than the new bellwether (aka GENERAL) method. We also have updated another research question (RQ3) to compare the methods in-term of number of comparisons and time taken to run the two methods, as can been seen in Figures~\ref{fig:compare_time} and
\ref{fig:compare}.}

 

\review{(1.6b) What makes the reported method scalable?}

\response{Thank you for your comment, in this version we have updated the RQ3 to show that our method is scalable. We have showcased the analysis of number of comparisons and run time between the two algorithms (conventional bellwether and GENERAL) using various community sizes.}
...
\response{Thank you very much for your detailed review.
As we said to the associated editor, the prior draft  assumed that the core motivation for this work was widely known and accepted in the community. That was a mistake so this draft completely rewrites the Introduction to include much more background information on the presence of the problem studied here. Also, thanks to your feedback, 
we have made some additional corrections, clarifications, and revisions, as directed by your comments. Please note that to assist in your review, we refer to specific locations of each of the changes using the notation \respto{2-XX}.}


\response{Before going on, we need to ask you one thing.
\begin{itemize}
\item
Several   of your comments take issue with two terms used in this paper ``lessons learned'' and "transfer learning".
We agree with you that our use of ``lessons learned" was inappropriate and we have removed all terms from this paper.
\item
But as to your argument that our use of ``transfer learning'' is  incorrect, that is not clear to us. 
As near as we can tell, our use of transfer learning is compatible with standard definitions in that field, both outside
of SE (e.g. Pan's paper~\cite{pan2009survey})
and within the SE literature~\cite{ma2012transfer,zimmermann2009cross,turhan2009relative,krishna2018bellwethers,jing2015heterogeneous,he2014towards,nam2017heterogeneous,cheng2016heterogeneous,yu2017feature}.
Also, reading through your comments, one thing that strikes us is that perhaps you are reserving the use of ``transfer learning''   only  to heterogeneous learning (where the source and target have different attributes)?  If so, then perhaps 
  \S\ref{tion:how1} is relevant since, in that section, we carefully define and distinguish between homogeneous and heterogeneous transfer learning and clearly state that this paper 
is about homogeneous learning, where the attributes are the same in source and target.   
\item
That said, we want to work with you on this point. Is there another term you would accept that is not ``lessons learned'' or ``transfer learning''?
Would the terms ``generalize'' or ``extend'' work for you? Please advise.
\end{itemize}} 
...
\begin{quote}
    \response{(a) `Given a community of projects developed by the same organization,
the ``bellwether'' is a project or  a handful of projects
whose data yields the best analytic (i.e. defect prediction, effort estimation, etc) on all others.} 
\end{quote}
\begin{quote}
    \response{Also, to answer your second question, ``…the bellwether is equivalent to the other models'' means ``bellwether is an exemplar project for the community of N projects seen so far''  }
\end{quote}
...
\response{You are correct. We have deleted Figure 1 from this manuscript. }
...
by  Figures~\ref{fig:compare_time} and ~\ref{fig:compare}.) }
...
\response{Yes, are correct. That was our mistake. All our current numbers now say 697.}
...
\response{You are correct-- that text was confusing. In this reworked version, that text is removed. .}
...
\response{
Thank you for that comment. We have deleted it.}
...
\response{Thank you for your comment, the Figure Figure 6 has been removed from this version. We have included Figure~\ref{fig:compare_time} which shows the time taken by traditional bellwether and GENERAL (without CFS and SMOTE) with increasing project community size. these two figures are important to showcase the scalability of the new bellwether method aka GENERAL.}


\section*{Reviewer 3}

\response{Thank you very much for your detailed review. As we said to the associated editor, the prior draft  assumed that the core motivation for this work was widely known and accepted in the community. That was a mistake so this draft completely rewrites the Introduction to include much more background information on the presence of the problem studied here. Also, thanks to your feedback, 
we have made some additional corrections, clarifications, and revisions, as directed by your comments. Please note that to assist in your review, we refer to specific locations of each of the changes using the notation \respto{3-XX}.}

\review{(3.1)The manuscript introduces a new approach GENERAL, which just applies hierarchical clustering. As far as I understand, hierarchical clustering is a well-known, standard, and largely-evaluated clustering algorithm. I don’t really see the need to re brand it with a new name. What is the difference between GENERAL and hierarchical clustering other than the GENERAL is applied to defect prediction?}
...
\response{We are not very clear on this comment.
In our view,   bellwethers are a method that has been demonstrated useful in many domains. 
As shown by our precision, recall, false alarm, etc results, this paper
shows that we can make general conclusions across this space of projects.}

\response{Also, in other papers\cite{krishna2018bellwethers,krishna2017simpler, nair19a, krishna16, krishna16a}, we have shown that
 bellwethers have worked to generate:
\begin{itemize}
\item
configurations for cloud-based tools~\cite{krishna2019whence}
(this recently got accepted to TSE);
\item 
effort estimation;
\item
predicting Github issue close time, 
\item
...
\response{
However, and this is the point of this paper, the existing  method does not scale to large data sets. Hence, this paper.}
...
\response{Thank you for your comment. In this study, ZeroR is a method where we predict everything as majority class. This effectively means for any project where in the training data majority of the modules are defective, all test data will be predicted as defective (increasing pf and lower precision) and vice-versa if majority of the modules are non-defective. The dataset we are working with more then 50\% of the projects has less than 50\% defective modules. So ZeroR will detect no defects in all these projects (Figure~\ref{fig:Statistical} show the median recall is zero precisely for this reason). This is the reason we can't use ZeroR as a recommended model.
We have added notes on that in \S\ref{sec:isk} and \citeresp{3-9}.}
...
\response{Thank you for your comments, in the last manuscript we did not properly explained why the model ZeroR can not be recommend. We have updated the Figure~\ref{fig:Statistical} to replace arithmetic mean and standard deviation with median and IQR. Figure~\ref{fig:Statistical} shows ZeroR model has a median recall and pf of zero with IQR diff between 75th and 25th percentile as 100. This result signifies the model either predicts everything as defective or non-defective. This is because ZeroR is a method where we predict everything as majority class. This effectively means for any project where in the training data majority of the modules are defective, all test data will be predicted as defective (increasing pf and lower precision) and vice-versa if majority of the modules are non-defective. In the dataset that we have used more then 50\% of the projects has less than 50\% defective modules, thus in result the median recall, precision and pf is zero.}
...
\response{Thank you for your valuable comments, the text for section~\ref{sec:rq6} was indeed confusing. We have removed that confusing terminology.}
